...
### Generating VAPID Keys (Required)

To use Web Push notifications, you must generate valid VAPID keys. The API server will crash on startup if these are invalid or missing. Generate them by running:

```bash
npx web-push generate-vapid-keys
```

Copy the generated `Public Key` and `Private Key` into your `apps/api/.env` file for `VAPID_PUBLIC_KEY` and `VAPID_PRIVATE_KEY` respectively.

*Note: For local development, the default placeholder values in the examples will work fine for most fields, except for VAPID keys which must be valid base64 strings.*
