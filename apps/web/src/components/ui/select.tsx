...
import { formatDisplayValue } from "@/lib/labels";
...
function Select<Value = any, Multiple extends boolean | undefined = false>({
  ...
  const [itemsMap] = React.useState(() => new Map<string, React.ReactNode>());
  const [, forceUpdate] = React.useReducer((x) => x + 1, 0);

  const registerItem = React.useCallback(
    (val: any, label: React.ReactNode) => {
      if (val != null) {
        const key = String(val);
        if (itemsMap.get(key) !== label) {
          itemsMap.set(key, label);
          forceUpdate();
        }
      }
    },
  ...
  return (
    <SelectContext.Provider value={{ itemsMap, registerItem }}>
      <SelectPrimitive.Root
        itemToStringLabel={(val: any) => {
          if (val == null) return "";
          const key = String(val);
          const registered = itemsMap.get(key);
      ...
      {typeof children === "function"
        ...
        : (children ??
          ((val: any) => {
            ...
            const key = String(val);
            if (ctx?.itemsMap.has(key)) {
              return ctx.itemsMap.get(key);
            }
            return formatDisplayValue(val);
          }))}
    </SelectPrimitive.Value>
