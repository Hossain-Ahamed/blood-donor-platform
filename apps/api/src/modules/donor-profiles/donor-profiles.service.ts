...
export class DonorProfilesService {
  ...
  async getMe(userId: string): Promise<DonorProfile> {
    const profile = await this.donorProfileRepository.findOne({ where: { user_id: userId } });
    if (!profile) {
  ...
  async updateMe(userId: string, dto: UpdateDonorProfileDto): Promise<DonorProfile> {
    ...
    if (dto.last_donation_date !== undefined) {
      profile.last_donation_date = dto.last_donation_date ? new Date(dto.last_donation_date) : null;
